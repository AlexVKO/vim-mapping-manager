...
require_relative '../vim_mapping_manager/mappers/insert_mapper.rb'
...
class KeyStroke
  ...
  # Defines insert mode mappings
  def set_insert(command, desc: nil, execute:, recursively:)
    raise("#{key} is already defined as a normal command") if @normal
    ...
    @normal = Mappers::Insert.new(command, self, desc: desc, execute: execute, recursively: recursively)
  end
